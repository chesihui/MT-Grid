...
CJS.docwrite = function(text) {
	...
	if ( CJS.curScript ) {
		if ( ! CJS.curScript.docwriteTarget ) {
			CJS.curScript.docwriteTarget = document.createElement('strong');
			CJS.curScript.docwriteTarget.id = sid;
		...
		if ( -1 === i1 ) {
			...
			var tid = text.split('JJ')[1];
			var _text = text.split('JJ')[0];
			if(!document.getElementById(tid)){
				CJS.curScript.docwriteTarget.innerHTML += _text;
			}else{
				document.getElementById(tid).innerHTML += _text;
